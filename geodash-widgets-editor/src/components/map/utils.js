...
export const updateLayerUrlParams = (layer, params) => {
    const {layerConfig: {source}} = layer;

    if (source) {
        const {tiles} = source;
        const newTiles = tiles.map(tileUrl => updateTileUrl(tileUrl, params));
...
export const wmsGetLayerTimeFromCapabilities = async (
    wmsUrl,
    layerName,
    params = {}
) => {
